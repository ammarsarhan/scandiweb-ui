            ...
            <div data-testid={`product-attribute-${convertToKebabCase(this.props.name)}`}>
                ...
                <div className='flex flex-wrap gap-2'>
                    {this.props.items.map((selections: object, index: number) => {
                        ...
                        if (index === this.state.selected) {
                            if (this.props.isClickable) {
                                return <button data-testid={`product-attribute-${selection.displayValue}`} className='border-[1px] p-1 border-black bg-black text-white text-sm' key={index} onClick={() => this.handleOptionClicked(index)} >{selection.displayValue}</button>
                            }
                            ...
                            if (this.props.isClickable) {
                                return <button data-testid={`product-attribute-${selection.displayValue}`} className='border-[1px] p-1 border-black text-sm' key={index} onClick={() => this.handleOptionClicked(index)}>{selection.displayValue}</button>
                            }
                            return <div 
